...
```mermaid
flowchart LR
    Caller[Calling application<br/>HTTP or TypeScript client] -->|Bearer API key| API[Fastify /v1 API]
...
| `src/http.ts` | Authenticates every `/v1` route, validates bodies and IDs, maps lifecycle errors to documented HTTP responses, and exposes `/health` and `/ready`. |
...
1. The caller sends `POST /v1/requests` with an idempotency key and the exact action description. Validation bounds text, metadata, details, and expiry. The core stores the canonical content and its fingerprint before attempting Telegram delivery. A repeat with the same key and content returns the existing row; changed content returns a conflict.
2. The Telegram worker reads due rows from SQLite, sends a message to the configured chat, and records the returned message ID. The message shows the action, title, description, details, short request ID, and expiry. Metadata stays with the authenticated API response and is not sent to Telegram. Transient send errors update the row to `retrying` with bounded backoff; a terminal error is visible as `failed`.
...
4. The caller reads `GET /v1/requests/:id` or uses `waitForDecision`. On approval, it calls `POST /v1/requests/:id/claim`. One conditional SQLite update moves execution from `unclaimed` to `claimed`; other claim attempts get a conflict. The response contains a one-time claim token. Only its hash is stored.
...
The initial schema is in [`migrations/001_initial.sql`](../migrations/001_initial.sql). It has three tables and no user-account or workflow tables.

| Table | Main fields | Purpose |
| --- | --- | --- |
| `requests` | `id`, unique `idempotency_key`, `fingerprint`, `content_json`, action/title/description/details/metadata, timestamps | One immutable proposal. The canonical content and fingerprint enforce idempotency across restarts. |
...
- The API key protects request creation, reads, cancellation, claims, and results. `/health` and `/ready` are public and return no credentials. The configured Telegram chat and allowlisted numeric user IDs control button decisions; usernames and forwarded messages do not grant authority.
...
`npm test` uses temporary SQLite databases, an injected clock, a fake Telegram transport, and an injected HTTP fetcher. It covers lifecycle and expiry boundaries, restart recovery, idempotency, API validation and authentication, Telegram authorization and callback binding, delivery retries and failure, polling offset replay, claim races, and client waiting. `npm run test:coverage` prints a source coverage report. Tests never use a real bot token or contact Telegram.
