...
JaGate is one Node.js process with a versioned HTTP API, a core request lifecycle, a SQLite database, and a Telegram adapter. A caller proposes an action and later performs it itself. The gateway stores intent, routes it to that client's Telegram chat, records a human decision, and coordinates a single claim; it never executes the action.
...
flowchart LR
    ...
    Worker <--> Transport[One bot transport]
    Transport <--> Telegram[Telegram Bot API]
    Telegram <--> Chats[Per-client chats and approvers]
...
2. The Telegram worker reads due rows from SQLite, resolves each row's client ID to its configured chat, sends a message, and records the chat and message IDs. The message shows the client ID, action, title, description, details, short request ID, and expiry. Metadata stays with the owning client's API response and is not sent to Telegram. Transient send errors update the row to `retrying` with bounded backoff; a terminal error is visible as `failed`.
3. One bot long poller receives button callbacks from all chats. The adapter resolves the opaque callback reference to a client, then checks that client's numeric approver user ID and current destination chat ID. The core checks the reference against the stored chat and message IDs and conditionally updates a still-pending, unexpired request. Repeated, late, wrong-chat, or wrong-message callbacks cannot reverse a decision. The adapter answers the callback and tries to edit the message to remove its buttons.
...
- After a destination change, startup requeues still-pending delivered rows, rotates their callback references, and sends them to the new chat. Old buttons fail closed. A migration from the earlier schema also requeues still-pending delivered rows because their prior chat ID was not stored. Settled decisions stay unchanged; the original expiry deadline stays in force. Changes to a client's approver allowlist take effect for existing pending buttons after restart.
